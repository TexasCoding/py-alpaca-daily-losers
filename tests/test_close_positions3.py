...
from alpaca_daily_losers.close_positions import (
    ClosePositions,  # assuming the refactored class is in close_positions.py
)
...
def close_positions(mock_trading_client, mock_stock_client, mock_logger):
    return ClosePositions(
        trading_client=mock_trading_client, stock_client=mock_stock_client, py_logger=mock_logger
    )
...
def test_sell_positions_from_criteria_with_sell_opportunities(close_positions, mock_trading_client):
    ...
    mock_positions_df = pd.DataFrame({"symbol": ["AAPL", "MSFT", "Cash"], "qty": [10, 20, 0]})
...
def test_get_stocks_to_sell(mock_get_ticker_data, close_positions, mock_trading_client):
    mock_positions_df = pd.DataFrame({"symbol": ["AAPL", "MSFT"], "profit_pct": [15.0, -10.0]})
    mock_trading_client.positions.get_all = MagicMock(return_value=mock_positions_df)
...
def test_sell_stocks_with_sell_list(close_positions, mock_trading_client):
    ...
    expected_result = [{"symbol": "AAPL", "qty": 10}, {"symbol": "MSFT", "qty": 20}]
...
@patch("alpaca_daily_losers.close_positions.get_ticker_data")
def test_get_stocks_to_sell_no_sell_criteria(
    mock_get_ticker_data, close_positions, mock_trading_client
):
    mock_positions_df = pd.DataFrame({"symbol": ["AAPL", "MSFT"], "profit_pct": [5.0, 2.0]})
    mock_trading_client.positions.get_all = MagicMock(return_value=mock_positions_df)
