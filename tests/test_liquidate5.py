...
from alpaca_daily_losers.liquidate import Liquidate  # assuming the module name is liquidate.py
...
def test_get_top_performers():
    ...
    assert result["symbol"].tolist() == expected["symbol"].iloc[: len(current_positions[current_positions["symbol"] != "Cash"]) // 2].tolist()


def test_liquidate_positions_sufficient_cash(liquidate, mock_trading_client):
    current_positions = pd.DataFrame({
        "symbol": ["AAPL", "MSFT", "Cash"],
        "market_value": [700.0, 800.0, 2000.0]
    })
...
def test_liquidate_positions_insufficient_cash(liquidate, mock_trading_client):
    ...
    liquidate.get_top_performers = MagicMock(return_value=current_positions[current_positions["symbol"] != "Cash"])
    liquidate._sell_top_performers = MagicMock(return_value=[{"symbol": "AAPL", "notional": round(134.0, 2)}])

    with patch('src.alpaca_daily_losers.global_functions.send_position_messages') as mock_send_position_messages:
        liquidate.liquidate_positions()
...
def test_sell_top_performers(liquidate, mock_trading_client):
    top_performers = pd.DataFrame({
        "symbol": ["AAPL", "MSFT"],
        "market_value": [500.0, 1000.0]
    })

    result = liquidate._sell_top_performers(top_performers, top_performers["market_value"].sum(), 150.0)
    expected_result = [
        {"symbol": "AAPL", "notional": 50},
        {"symbol": "MSFT", "notional": 100}
    ]
...
def test_sell_top_performers_with_zero_to_sell(liquidate, mock_trading_client):
    ...
    result = liquidate._sell_top_performers(top_performers, top_performers["market_value"].sum(), 1.0)
    assert not result
